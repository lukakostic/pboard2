...
function boardExtensionChecked(event){
  ...
  //pb.boards[extrasSelected].attributes['extensions'].splice(findWithAttr(getBrdAttr(extrasSelected, 'extensions'),'id',id),1);
...
function extensionsDialogSearched(){
  let s = (<HTMLInputElement> EbyId('extensionsDialogSearch')).value;
