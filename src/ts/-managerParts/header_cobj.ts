...
class _Header_{
   ...
   constructor(){
      ...
      (this.saveBtn = EbyId('saveBtn')).onclick = (click:MouseEvent)=>{if(click.shiftKey || click.ctrlKey) sync.saveAll(); else sync.saveDirty();};
   ...
   headerTitle_oninput(){
      ...
      boardsUpdated(UpdateSaveType.AutoSave, mainView.id,[mainView.id]);
   }
