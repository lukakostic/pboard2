...
let allDialogs: {[index:string]:(new(back:HTMLElement,dialog:HTMLElement)=>DialogInterface)} = {};
...
class _DialogManager_{
  ...
  constructor(){
	...
	openDialog(dialogName :string, boardId :BoardId, boardView :View) :void{
		if(dialogName in allDialogs == false) return;
		...
		let dialog = new allDialogs[dialogName](dialogEl,back);
